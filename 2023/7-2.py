...
class Hand:
    ...
    def get_type(self, cards):
        if any(c == "J" for c in cards):
            cards = self.get_best_joker_hand(cards)
    ...
    def get_best_joker_hand(self, cards):
        cards_minus_joker = "AKQT98765432"
        joker_positions = [idx for idx, v in enumerate(cards) if v == "J"]

        best_hand = (-1, -1)
        top_hand = None

        # cartesian product of every possible card for each position
        for potential_jokers in itertools.product(
            *[cards_minus_joker for _ in range(len(joker_positions))]
        ):
            h = list(cards)
            for i, j in enumerate(joker_positions):
                h[j] = potential_jokers[i]
            if rank(h) > best_hand:
                best_hand = rank(h)
                top_hand = h
...
# return tuple representing the type, then card value of hand
def rank(cards):
    t = None
    d = {}
    for c in cards:
    ...
    if len(d.keys()) == 1:
        ...
        t = "high_card"
    return (TYPE_RANKINGS[t], sum([CARD_RANKINGS[c] for c in cards]))
